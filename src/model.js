...
export const generalNews = async function () {
    try {
    const data = await fetch('https://blog-mock-api.onrender.com/api/v1/news').then(res => res.json())
...
export const typeNews = async function (source) {
    try { 
      ...
      if (!source)  throw new error;
        store.source = source;
        const data = await fetch(`https://blog-mock-api.onrender.com/api/v1/news/type/${source}`).then(res => res.json())
        createObject(data)
    } catch (error) {
        throw err;
    }
...
export const getContentDetails = async function (id) {
  try {
    ...
    const data = await fetch(`https://blog-mock-api.onrender.com/api/v1/news/${id}`).then(res => res.json())
...
export const categoryNews = async function (id) {
  try {
    const data = await fetch(`https://blog-mock-api.onrender.com/api/v1/news/?category=${id}`).then(res => res.json())
    createObject(data)
    // console.log(store.search.result)
  } catch (err) {
    console.log(err)
